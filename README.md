...
Cobot supports connecting to Feishu (China) or Lark (International) as a messaging channel. Setup is fully interactive:

```bash
# Interactive QR-code setup (recommended)
cobot setup feishu

# Interactive manual setup with existing app credentials
cobot setup feishu --manual

# Specify channel name and domain
cobot setup feishu --name my-feishu --domain lark
```

The command will:
1. Open a QR code URL — scan with the Feishu/Lark mobile app to register a new bot
2. Verify bot connectivity
3. Ask for your gateway address (default `:8080`)
4. Auto-generate a `verification_token` and `encrypt_key`
5. Print the webhook URL to paste into the Feishu Open Platform

After setup, your `config.yaml` contains the channel entry:
...
channels:
  - name: default
    ...
    config:
      ...
      encrypt_key: <auto-generated>
      domain: feishu   # or "lark" for international
```

To enable events in the Feishu Open Platform:
1. Go to **Events & Callbacks → Request URL**
2. Paste the webhook URL printed at the end of setup
3. Enter the `verification_token` when prompted
4. Enable **AES encryption** and paste the `encrypt_key`
5. Subscribe to the **Receive Messages** event

Start the gateway with:

```bash
cobot gateway --addr :8080
```
