...
func TestMemorySearchTool(t *testing.T) {
	...
	id, err := s.Store(ctx, "decided to use BadgerDB for storage", wing.ID, room.ID)
	if err != nil {
		t.Fatal(err)
	}
	_ = id
	...
	result, err := tool.Execute(ctx, []byte(`{"query":"BadgerDB"}`))
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(result, "BadgerDB") {
		t.Errorf("expected result to contain 'BadgerDB', got: %s", result)
	}
...
func TestMemorySearchToolWithWingFilter(t *testing.T) {
	...
	s.Store(ctx, "alpha content about BadgerDB", wing1.ID, room1.ID)
	s.Store(ctx, "beta content about BadgerDB", wing2.ID, room2.ID)

	time.Sleep(100 * time.Millisecond)

	tool := NewMemorySearchTool(s)
	result, err := tool.Execute(ctx, []byte(`{"query":"BadgerDB","wing_id":"`+wing1.ID+`"}`))
	if err != nil {
		t.Fatal(err)
	}
