...
type WorkspaceConfig struct {
	...
	Sandbox       WSSandboxConfig      `yaml:"sandbox,omitempty"`
...
func (w *Workspace) AgentsMdPath() string {
	root := w.Definition.Root
	if root == "" {
		return ""
	}
	return filepath.Join(root, ".cobot", "AGENTS.md")
}
...
func (w *Workspace) ToolsConfigPath() string {
	if w.Definition.Root != "" {
		return filepath.Join(w.Definition.Root, ".cobot", "tools.yaml")
	}
	return filepath.Join(w.DataDir, "tools.yaml")
}
...
func EnsureDefaultWorkspace() (*Workspace, error) {
	...
	dataDir := xdg.DataDir()
	defPath := filepath.Join(definitionsDir, "default.yaml")
	...
	ws := newWorkspaceFromDefinition(def, dataDir)
