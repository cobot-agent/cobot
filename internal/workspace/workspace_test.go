...
func TestWorkspace_EnsureDirs(t *testing.T) {
	...
	expectedDirs := []string{
		...
		filepath.Join(dataDir, "scheduler"),
	}
...
func TestWorkspace_Accessors(t *testing.T) {
	...
	if ws.SchedulerDir() != filepath.Join(dataDir, "scheduler") {
		t.Errorf("SchedulerDir() = %s", ws.SchedulerDir())
	}
	if ws.ConfigPath() != filepath.Join(dataDir, "workspace.yaml") {
		t.Errorf("ConfigPath() = %s", ws.ConfigPath())
	}
	expectedAgentsMd := filepath.Join("/home/user/project", ".cobot", "AGENTS.md")
	if ws.AgentsMdPath() != expectedAgentsMd {
		t.Errorf("AgentsMdPath() = %s, want %s", ws.AgentsMdPath(), expectedAgentsMd)
	}

	wsNoRoot := &Workspace{
		Definition: &WorkspaceDefinition{Name: "x", Type: WorkspaceTypeDefault},
		Config:     &WorkspaceConfig{ID: "x", Name: "x", Type: WorkspaceTypeDefault},
		DataDir:    dataDir,
	}
	if wsNoRoot.AgentsMdPath() != "" {
		t.Errorf("AgentsMdPath() should be empty when no root, got %s", wsNoRoot.AgentsMdPath())
	}
}
