...
func runSetupFeishu(cmd *cobra.Command) error {
	...
	fmt.Println("  1. Enable message events in Feishu Open Platform → Events & Callbacks")
	fmt.Println("     - Enable \"Receive Messages\"")
	fmt.Println("     - Set Request URL to: https://your-server/webhook/feishu/<channel-name>")
	fmt.Println("  2. Run the gateway:")
	fmt.Printf("       cobot gateway\n")
	fmt.Println()
	return nil
}
